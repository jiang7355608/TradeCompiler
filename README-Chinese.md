...
- 价格 > rangeLow（未跌破）
- K线确认：收阳 + 收盘价 > 前一根收盘价
→ 开试探仓：10% 基础仓位 × 0.60 置信度 = 6% 实际仓位
→ 虚拟止损：入场价 - 15% 箱体宽度（策略监控，不下到交易所）
→ 无交易所止损止盈（策略监控 3 小时超时或虚拟止损触发）
```
...
- K线确认：收阴 + 收盘价 < 前一根收盘价
→ 开试探仓：10% 基础仓位 × 0.60 置信度 = 6% 实际仓位
→ 虚拟止损：入场价 + 15% 箱体宽度（策略监控）
→ 无交易所止损止盈
```

**阶段2：加仓（整仓止损止盈，attachAlgoOrds 原子绑定）**
```
触发条件：
- 浮盈 ≥ 箱体宽度的 25%
- 持仓时间 ≥ 45 分钟

→ 加仓：30% 基础仓位 × 0.85 置信度 = 25.5% 实际仓位
→ 总仓位：约 31.5%（试探仓 + 加仓）
→ 整仓止损：保本位（入场价 + 5% 箱体宽度保护）
→ 整仓止盈：箱体中线
→ 止损止盈通过 OKX attachAlgoOrds API 原子绑定到整个持仓
→ 交易所自动执行，无需策略再监控
```
...
```
试探仓：6% 实际仓位（10% 基础 × 0.60 置信度）
加仓：25.5% 实际仓位（30% 基础 × 0.85 置信度）
总仓位：约 31.5% 最大持仓
